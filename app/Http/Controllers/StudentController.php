...
class StudentController extends Controller
{
    public function show($id = null)
    {
        ...
        [$blok, $fbmsActive, $student, $vakkenActiveB] = self::getStudentScoresForBlok(
            $groupFromApi,
            $cohortId,
    ...
    private function getStudent($id = null)
    {
        ...
        if ($id == null || Auth::user()->type == 'student') {
            return collect(SdApi::get('/me'));
        }
    ...
    public static function getStudentScoresForBlok($group, $cohortId, $onlyForUser = null, $blokId = -1, $vakkenToHide = [])
    {
        ...
        $blok->vakken = collect($blok->vakken)
            ->filter(function ($vak) {
                return ! empty($vak->feedbackmomenten); // exclude vakken without feedbackmoment
            })
        ...
        $studentIds = collect($group['users'])
            ->pluck('id');
        $fbmIds = StudentScore::whereIn('student_id', $studentIds)
            ->distinct()
            ->pluck('feedbackmoment_id');
        ...
        $vakkenActiveB = DB::table('student_scores_b')
            ->whereIn('student_id', $studentIds)
            ->whereIn(
                'subject_id',
                $blok->vakken->pluck('uitvoer_id')
            )
            ->select('subject_id')
            ->distinct()
            ->pluck('subject_id');
