<table class="table-auto border-collapse border border-gray-400">
    <thead class="bg-white shadow">
        ...
        <tr>
            <x-table.th disabled class="sticky left-0 italic text-sm text-right font-normal pe-2">acties:</x-table.th>
            @foreach ($this->blok->vakken as $vak)
                @foreach ($vak->feedbackmomenten as $fbmKey => $feedbackmoment)
                    <x-table.thfbm :loop="$loop" :fbmsActive="$fbmsActive" :currentWeek="$currentWeek" :feedbackmoment="$feedbackmoment" wire:click="startFloodFill({{ $feedbackmoment->id }})">
